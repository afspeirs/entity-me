<script lang="ts">
  ...
  async function getEntities(category: CategoryValue): Promise<Entity[]> {
  ...
  const perPage = 100;
  const filteredItemsPromise = $derived((async () => {
    // Read search.value FIRST to establish it as a dependency
    const currentSearchValue = search.value;

    const allEntities = await getEntities(currentCategory.value);
    if (!allEntities || allEntities.length === 0) return [];
    return allEntities.filter((item) =>
      item.character?.includes(currentSearchValue) ||
      item.decimal?.includes(currentSearchValue) ||
      item.hex?.includes(currentSearchValue) ||
      item.entity?.includes(currentSearchValue) ||
      item.description?.toLowerCase()?.includes(currentSearchValue.toLowerCase()) ||
      item.note?.toLowerCase()?.includes(currentSearchValue.toLowerCase()),
    );
  })());
...
<div class="flow-root min-w-full align-middle px-safe pb-safe">
  <div class="shadow-sm ring-1 ring-black/5">
    <table class="min-w-full border-separate border-spacing-0 divide-y divide-gray-300">
      ...
      <tbody class="divide-y divide-gray-200 bg-white dark:bg-dark">
        ...
        {:then filteredItems}
          ...
          {@const paginatedEntities = filteredItems.slice(start, end)}
    ...
    {#await filteredItemsPromise then filteredItems}
      <TablePagination totalItems={filteredItems.length} {perPage} bind:currentPage />
    {/await}
  </div>
</div>
